...
describe("token-synchronize", () => {
  ...
  async function create_mint_with_extensions_v2(
    ...
    transaction.add(
      createInitializeMint2Instruction(mint.publicKey, decimals, mint_authority, null, TOKEN_2022_PROGRAM_ID)
    );
  ...
  // // describe("get_account_data_size", () => {
  // //   it("Calculates account data size correctly", async () => {

  // //     // const { mint } = await create_and_mint_token(TOKEN_2022_PROGRAM_ID, 6);
  // //     const extension = [ExtensionType.TransferFeeConfig];
  // //     const mint = await create_mint_with_extensions_v2(connection, wallet.payer, wallet.publicKey, extension, 1, BigInt(1), 6)

  // //     const mintInfo = await getMint(connection, mint, undefined, TOKEN_2022_PROGRAM_ID);

  // //     console.log(mintInfo)
  // //     // // The program expects a Vec<u8>, so we pass the numeric values.
  // //     const extension_type = [1]; 

  // //     // // const { result } = await program.methods
  // //     // //   .getAccountDataSize(extension)
  // //     // //   .accounts({
  // //     // //     tokenProgramId: TOKEN_2022_PROGRAM_ID,
  // //     // //     mint: mint,
  // //     // //   })
  // //     // //   .simulate();
  // //     // // const calculatedSize = result;

  // //     // // const expectedSize = getAccountLen(extension);
  ...
  // //     // assert.equal(calculatedSize.toString(), expectedSize.toString());
  // //   });
  // // });
  ...
  describe("transfer_checked", () => {
    ...
    it("Transfers Token-2022 tokens", async () => {
        ...
        const toAccount = await createAccount(connection, wallet.payer, mint, wallet.publicKey, undefined, undefined, TOKEN_2022_PROGRAM_ID);
        ...
        assert.equal(Number(fromAccountInfo.amount), mintAmount - amount);
        assert.equal(Number(toAccountInfo.amount), amount);
    });

  //   it("Transfers Token-2022 tokens with fee", async () => {
  //       const mint = anchor.web3.Keypair.generate();
  //       const transferFeeBasisPoints = 100; // 1%
  //       const maximumFee = BigInt(1000);
  ...
  //       const toAccount = await createAccount(connection, wallet.payer, mint.publicKey, wallet.publicKey, undefined, undefined, TOKEN_2022_PROGRAM_ID);
  //       const mintAmount = 10000;
  //       await mintTo(connection, wallet.payer, mint.publicKey, fromAccount, wallet.publicKey, mintAmount, [], undefined, TOKEN_2022_PROGRAM_ID);

  //       const transferAmount = 1000;
  //       const fee = calculateFee(BigInt(transferFeeBasisPoints), BigInt(transferAmount));
  ...
  //       assert.equal(Number(fromAccountInfo.amount), mintAmount - transferAmount);
